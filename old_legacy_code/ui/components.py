...
class FormField:
    ...
    def __init__(
        ...
        if field_type == "entry":
            ...
            # Enable text undo/redo for entry fields
            self._setup_text_undo(self.widget)
    ...
    def _setup_text_undo(self, entry_widget):
        """Setup text undo/redo for entry fields"""
        # Configure undo with max 20 operations
        try:
            # Get the underlying tk entry widget
            tk_entry = entry_widget._entry
            tk_entry.configure(undo=True, maxundo=20)
            
            # Bind Ctrl+Z for undo
            entry_widget.bind('<Control-z>', lambda e: self._text_undo(tk_entry))
            # Bind Ctrl+Y for redo
            entry_widget.bind('<Control-y>', lambda e: self._text_redo(tk_entry))
        except Exception:
            # Fallback if undo not supported or widget structure different
            pass
    
    def _text_undo(self, tk_widget):
        """Undo text in entry field"""
        try:
            tk_widget.edit_undo()
        except Exception:
            pass
    ...
    def _text_redo(self, tk_widget):
        """Redo text in entry field"""
        try:
            tk_widget.edit_redo()
        except Exception:
            pass
...
class MenuButton(ctk.CTkButton):
    ...
    def __init__(
        ...
        # Store original color for unhighlighting
